...
namespace LibraryManagementSystem
{
    public partial class ReturnBooks : UserControl
    {
        ...
        private void returnBooks_returnBtn_Click(object sender, EventArgs e)
        {
            ...
            // Confirmation dialog
            DialogResult check = MessageBox.Show($"Are you sure that Issue ID: {returnBooks_issueID.Text.Trim()} is already returned?",
            ...
            if (check == DialogResult.Yes)
            {
                using (SqlConnection connect = DatabaseConnection.GetConnection())
                {
                    try
                    {
                        ...
                        // Update query to set the status to "Return"
